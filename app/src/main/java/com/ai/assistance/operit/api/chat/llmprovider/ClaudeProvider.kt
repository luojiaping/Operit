...
class ClaudeProvider(
    ...
    /**
     * Thinking 格式模式。
     * - ADAPTIVE: thinking.type="adaptive" + output_config.effort（新模型）
    ...
    /**
     * 将 thinkingQualityLevel (1-4) 映射为 effort 等级
     * 1 → "low", 2 → "medium", 3 → "high"(默认), 4 → "max"
     */
    private fun resolveClaudeThinkingEffort(context: Context): String {
        val qualityLevel = runCatching {
            runBlocking {
                com.ai.assistance.operit.data.preferences.ApiPreferences
                    .getInstance(context).thinkingQualityLevelFlow.first()
            }
        }.getOrElse {
            AppLogger.w("AIService", "Failed to read thinking quality level, using app default")
            com.ai.assistance.operit.data.preferences.ApiPreferences.DEFAULT_THINKING_QUALITY_LEVEL
        }
