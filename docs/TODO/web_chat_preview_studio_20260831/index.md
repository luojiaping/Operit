...
- 视口修复：手机壳内改为 iframe 独立视口（376x816），补挂聊天样式表，
  100dvh 与媒体查询按手机尺寸计算
- App 复刻：iframe 内 AppShell（全局 TopBar + 导航抽屉 + 屏幕路由）、
  设置主页（8 分组 20 项）与全部设置子页骨架（20 页，主题页控件真联动）、
  消息上下文菜单与变体切换（mock 层，app 代码零改动）
