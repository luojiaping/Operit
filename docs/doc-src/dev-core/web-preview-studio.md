...
## 手机视口内的 App 复刻（iframe）

手机壳屏幕内是独立视口 iframe（376x816），由 AppShell 提供完整 App 体验：

- 全局 TopBar（64dp、primary 底、Menu/返回切换、标题"AI对话 - 会话标题"、
  AI电脑与 Web开发 action），主题联动
- 导航抽屉（宽 75% 屏、右侧 16dp 圆角）：品牌信息卡 + 网络胶囊 +
  三张 76dp 液态玻璃快捷卡 + AI 功能导航组 + 底部三卡（关于/使用手册/设置）
- 屏幕路由：聊天 / 设置主页（8 分组 20 项，文案对齐 app SettingsScreen）/
  全部设置子页（20 页骨架：主题页为核心控件真联动，其余为结构演示）
- 主题设置页 5 Tab（基础/背景/聊天/输入栏/界面栏）：模式、自定义配色、
  字号、背景与模糊、气泡与输入栏开关，改完手机即时生效
- 消息上下文菜单（右键，仅 mock 模式）：复制/信息/编辑并重发/回滚到此处/
  重新生成/删除（二段确认），以及消息变体切换 —— 全部走 mock 内存操作，
  真机 web 协议无对应端点，故真机入口不挂菜单
